...
def fee_per_share(price: float) -> float:
    ...
    return _POLICY["fee_rate"] * p * (1.0 - p)
...
def evaluate_market(
    ...
    *,
    ...
    if signal_ask < _POLICY["signal_ask_min"]:
    ...
    if arrival is None:
        ...
        return result
    arrival_ask = _finite(arrival.get("au" if selected_side == "Up" else "ad"))
    arrival_size = _finite(arrival.get("sau" if selected_side == "Up" else "sad"))
    ...
    cancel_floor = signal_ask - _POLICY["cancel_below_signal_by_more_than"]
    ...
    if arrival_ask > signal_ask + 1e-12:
        ...
        return result
    if arrival_size is None or arrival_size < _POLICY["shares"]:
        ...
        result.update(
            {
                ...
                "pnl_at_five_shares": pnl_per_share * _POLICY["shares"],
            }
        )
    return result
...
def build_market_ledger(asset: str, markets_path: Path, ticks_path: Path) -> pd.DataFrame:
    ...
    markets = pd.read_parquet(
        markets_path,
        columns=["condition_id", "event_id", "slug", "market_start", "market_end", "outcome"],
    )
    ...
    needed = {_POLICY["entry_second"], _POLICY["entry_second"] + _POLICY["latency_seconds"]}
    ...
    signal_second = _POLICY["entry_second"]
    arrival_second = signal_second + _POLICY["latency_seconds"]
...
def run_audit(candidate_spec: Path, out_dir: Path, cache_dir: Path) -> dict[str, Any]:
    ...
    eligible.to_csv(eligible_path, index=False, compression={"method": "gzip", "mtime": 0})
    ...
    decision_counts = {str(k): int(v) for k, v in ledger["decision"].value_counts(dropna=False).sort_index().items()}
    audit: dict[str, Any] = {
    ...
    report = f"""# Late-Favorite BTC/ETH Reproducibility Audit\n\n"
    ...
    report += f"- Markets: {len(ledger):,}\n- Signals: {int(ledger['signal'].sum()):,}\n- Hypothetical five-share FOK fills: {int(ledger['hypothetical_fok_fill'].sum()):,}\n"
    report += f"- All-condition-set SHA-256: `{audit['all_condition_ids_sha256']}`\n- Filled-condition-set SHA-256: `{audit['filled_condition_ids_sha256']}`\n\n"
    ...
    outputs = [audit_path, eligible_path, ledger_path, out_dir / "REPORT.md"]
...
def main() -> None:
    ...
    print(json.dumps(run_audit(args.candidate_spec, args.out_dir, args.cache_dir), indent=2, sort_keys=True, default=list))
