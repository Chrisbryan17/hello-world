#!/usr/bin/env python3
"""Independent verifier for a terminal-triangle cap witness."""
...
def double_cap(cap: tuple[int, ...], terminal: int) -> tuple[int, ...]:
    order = len(cap)
    graph = [0] * (2 * order)
...
def verify(witness: dict) -> dict:
    if witness["kind"] == "base_graph_is_counterexample":
        graph = graph_from_edges(int(witness["order"]), witness["base_edges"])
        ...
        return {
            ...
            "order": len(graph),
            "regular_degree": 3,
            "power_lengths_checked": check_powers(graph),
        }

    assert witness["kind"] == "terminal_triangle_cap"
    ...
    cap_checked = check_powers(cap)
    full = double_cap(cap, terminal)
    ...
    return {
        ...
        "counterexample_power_lengths_checked": check_powers(full),
    }
