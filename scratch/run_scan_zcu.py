...
TARGET = "0xb89d0b6e96e790afa900b53476b8f267a94d1d4f"
...
replacement = r'''def fetch_market_map() -> dict[str, dict[str, Any]]:
    ...
    while cursor <= final:
        ...
        offset = 0
        while True:
            page = get_json(session, f"{GAMMA}/markets", {
                "closed": "true", "tag_id": 21,
            ...
            for m in page:
                ...
                if winner and START <= en <= NOW:
                    result[str(m.get("conditionId", ""))] = {
                        "slug": slug, "title": m.get("question") or m.get("title") or slug,
                        "start": st, "end": en, "duration": duration, "winner": winner,
